class ThemeGenerator < Rails::Generator::Base
  ...
  def manifest
    record do |m|
      ...
      %w(cross key tick application_edit bin_empty download house_go find pdf xml json csv help page_white_acrobat).each do |icon|
...
protected
  ...
  def add_options!(opt)
    ...
    opt.on("--jrails=true", "Use JRails") { |v| options[:jrails] = true }
    opt.on("--tinymce=true", "Use TinyMCE in textarea fields") { |v| options[:tinymce] = true }
    opt.on("--breadcrumb=true", "Add breadcrumbs to application layout") { |v| options[:breadcrumb] = true }
  end
